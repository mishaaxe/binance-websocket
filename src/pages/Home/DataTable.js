...
const Row = (props) => (
  <>
    <div className="d-none d-sm-inline">
      <div className="row table-row small py-1" to={`trade/${props.symbol}`}>
        <div className="col">{props.symbol}</div>
        <div className="col">
          {new BigNumber(props.lastPrice).toFormat(null, 1)}
        </div>
        <div
          className={
        ...
        >{`${new BigNumber(props.priceChangePercent).toFormat(2, 1)}%`}</div>
        <div className="col">
          {new BigNumber(props.highPrice).toFormat(null, 1)}
        </div>
        <div className="col">
          {new BigNumber(props.lowPrice).toFormat(null, 1)}
        </div>
        <div className="col">
    ...
    <div className="d-inline d-sm-none">
      <div className="row table-row small py-1" to={`trade/${props.symbol}`}>
        <div className="col-12">
          ...
          <span>{new BigNumber(props.lastPrice).toFormat(null, 1)}</span>{" "}
          <span
            className={
              props.priceChangePercent < 0 ? "text-danger" : "text-success"
            }
          >{`${new BigNumber(props.priceChangePercent).toFormat(2, 1)}%`}</span>
        </div>
        <div className="col-4">
...
const DataTable = (props) => {
  ...
  for (var i = 0; i < numRows; i++) {
    if (props.filter.includes(tickerArray[i].symbol)) {
      rows.push(<Row {...tickerArray[i]} key={tickerArray[i].symbol} />);
    }
  }
  return (
    <>
